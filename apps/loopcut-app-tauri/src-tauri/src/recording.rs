...
pub async fn get_available_audio_devices() -> Result<Vec<AudioDeviceInfo>, String> {
    ...
    let devices = loopcut_audio::core::device::list_audio_devices()
        .await
        .map_err(|e| format!("Failed to list audio devices: {}", e))?;

    let default_input = loopcut_audio::core::device::default_input_device()
        .map(|d| d.to_string())
        .ok();
    let default_output = loopcut_audio::core::device::default_output_device()
        .await
        .map(|d| d.to_string())
        .ok();
...
pub async fn get_available_monitors() -> Result<Vec<MonitorDevice>, String> {
    ...
    let monitors = loopcut_screen::monitor::list_monitors().await;
