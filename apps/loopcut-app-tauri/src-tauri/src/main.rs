...
async fn main() {
    ...
    let store_path = loopcut_core::paths::default_screenpipe_data_dir().join("store.bin");
    let store_json = std::fs::read(&store_path)
        ...
        .and_then(|data| {
            if data.len() >= 8 && &data[..8] == b"SPSTORE1" {
                ...
                let plain = loopcut_vault::crypto::decrypt_small(&data[8..], &key).ok()?;
    ...
    {
        let log_dir = loopcut_core::paths::default_screenpipe_data_dir();
    ...
    let sync_scheduler = loopcut_connect::sync_scheduler::SyncScheduler::new();
        ...
        .setup(move |app| {
            ...
            let base_dir = get_base_dir(app_handle, None)
                .unwrap_or_else(|e| {
                    ...
                    loopcut_core::paths::default_screenpipe_data_dir()
                });

            // Set up rolling file appender
            let log_dir = get_screenpipe_data_dir(app.handle())
                .unwrap_or_else(|_| loopcut_core::paths::default_screenpipe_data_dir());
                ...
                tauri::async_runtime::spawn(async move {
                    ...
                    if let Some(engine) = engine {
                        ...
                        tokio::task::spawn_blocking(move || {
                            match loopcut_audio::transcription::whisper::model::download_whisper_model(engine_clone) {
                    ...
                    let (_silero_result, _seg_result, _emb_result) = tokio::join!(
                        async {
                            match loopcut_audio::vad::silero::SileroVad::ensure_model_downloaded().await {
                                Ok(p) => info!("silero vad model pre-download complete: {:?}", p),
                                Err(e) => warn!("silero vad pre-download failed (will retry): {}", e),
                            }
                        },
                        async {
                            match loopcut_audio::speaker::models::get_or_download_model(
                                loopcut_audio::speaker::models::PyannoteModel::Segmentation
                            ).await {
                        ...
                        async {
                            match loopcut_audio::speaker::models::get_or_download_model(
                                loopcut_audio::speaker::models::PyannoteModel::Embedding
                            ).await {
            ...
            //         && loopcut_vault::crypto::is_encrypted_file(&data_dir.join("db.sqlite")).unwrap_or(false));
            // if vault_is_locked {
            ...
            // to avoid competing with Tauri's UI runtime
            {
                ...
                let on_pipe_output: Option<loopcut_core::pipes::OnPipeOutputLine> = Some(
