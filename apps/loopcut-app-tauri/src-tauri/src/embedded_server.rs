...
use loopcut_audio::core::device::{
    default_input_device, default_output_device, parse_audio_device,
};
use loopcut_audio::meeting_detector::MeetingDetector;
use loopcut_db::DatabaseManager;
use loopcut_engine::{
    analytics,
...
pub struct EmbeddedServerHandle {
    ...
    ui_recorder_handle: Option<loopcut_engine::UiRecorderHandle>,
    audio_manager: Option<std::sync::Arc<loopcut_audio::audio_manager::AudioManager>>,
    ...
    pipe_manager: Option<std::sync::Arc<tokio::sync::Mutex<loopcut_core::pipes::PipeManager>>>,
}
...
pub async fn start_embedded_server(
    ...
    on_pipe_output: Option<loopcut_core::pipes::OnPipeOutputLine>,
    ...
    let offline_mode = loopcut_core::offline::is_offline_mode();
    ...
    // Build audio manager
    use loopcut_audio::core::engine::AudioTranscriptionEngine;
    use loopcut_audio::transcription::stt::{
    ...
    // audio to live frames.
    {
        ...
        audio_manager.set_on_transcription_insert(std::sync::Arc::new(move |info| {
            ...
            rt.spawn(async move {
                cache
                    .push_audio(loopcut_engine::hot_frame_cache::HotAudio {
    ...
    let vision_metrics = Arc::new(loopcut_screen::PipelineMetrics::new());
    ...
    let initial_power_pref = config
        ...
        .as_deref()
        .and_then(|s| {
            serde_json::from_value::<loopcut_engine::power::PowerMode>(
    ...
    let mut capture_trigger_tx: Option<loopcut_engine::event_driven_capture::TriggerSender> =
        None;
    ...
    if !config.disable_audio {
        ...
        tokio::spawn(async move {
            ...
            if drm_pause && loopcut_engine::drm_detector::drm_content_paused() {
    ...
    let ui_recorder_handle = {
        ...
        .await
        {
            Ok(handle) => {
                ...
                loopcut_engine::drm_detector::set_ui_recorder_stop_flag(handle.stop_flag());
                Some(handle)
            }
            Err(e) => {
    ...
    let _speaker_id_handle =
        loopcut_engine::start_speaker_identification(db.clone(), config.user_name.clone());
    ...
    if config.schedule_enabled {
        loopcut_engine::schedule_monitor::start_schedule_monitor(
    ...
    loopcut_engine::start_snapshot_compaction(
        db.clone(),
    ...
    let pi_executor = std::sync::Arc::new(loopcut_core::agents::pi::PiExecutor::new(user_token));
    let mut agent_executors: std::collections::HashMap<
        String,
        std::sync::Arc<dyn loopcut_core::agents::AgentExecutor>,
    > = std::collections::HashMap::new();
    agent_executors.insert("pi".to_string(), pi_executor.clone());

    // Create pipe store backed by the main SQLite DB
    let pipe_store: Option<std::sync::Arc<dyn loopcut_core::pipes::PipeStore>> =
        Some(std::sync::Arc::new(
            loopcut_engine::pipe_store::SqlitePipeStore::new(db.clone()),
        ));

    let mut pipe_manager = loopcut_core::pipes::PipeManager::new(
        pipes_dir,
        agent_executors,
        pipe_store,
        config.port,
    ...
    tokio::spawn(async move {
        use loopcut_core::agents::AgentExecutor;
    ...
    if let Err(e) = loopcut_connect::mdns::advertise(config.port) {
...
mod tests {
    ...
    fn test_shutdown_sets_ui_recorder_stop_flag() {
        ...
        let handle = EmbeddedServerHandle {
            shutdown_tx,
            ui_recorder_handle: Some(loopcut_engine::UiRecorderHandle::new_for_test(flag)),
            audio_manager: None,
            pipe_manager: None,
        };
