...
pub async fn vault_status(_app: AppHandle) -> Result<String, String> {
    ...
    if db_path.exists() {
        if let Ok(true) = loopcut_vault::crypto::is_encrypted_file(&db_path) {
            return Ok("locked".to_string());
        }
    }
...
pub async fn vault_unlock(_app: AppHandle, password: String) -> Result<(), String> {
    use loopcut_vault::crypto;

    let screenpipe_dir = loopcut_core::paths::default_screenpipe_data_dir();
    ...
    tokio::spawn(async move {
        if data_dir.exists() {
            let (tx, _rx) =
                tokio::sync::watch::channel(loopcut_vault::migration::MigrationProgress {
                    total_files: 0,
                    processed_files: 0,
                    total_bytes: 0,
                    processed_bytes: 0,
            ...
            if let Err(e) =
                loopcut_vault::migration::decrypt_data_dir(&screenpipe_dir, &data_dir, key, tx)
                    .await
            {
