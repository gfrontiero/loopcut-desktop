...
use loopcut_core::agents::pi::screenpipe_cloud_models;
...
fn pi_local_install_dir() -> Option<PathBuf> {
    Some(loopcut_core::paths::default_screenpipe_data_dir().join("pi-agent"))
}
...
fn ensure_screenpipe_skill(project_dir: &str) -> Result<(), String> {
    use loopcut_core::agents::pi::PiExecutor;
    ...
    if bun_path != "NOT FOUND" {
        if let Some(bun_dir) = std::path::Path::new(&bun_path).parent() {
            ...
            let new_path = {
                ...
                let bash_result =
                    tokio::task::spawn_blocking(loopcut_core::agents::pi::ensure_bash_available)
                        .await
                        .unwrap_or_else(|e| {
...
pub async fn pi_update_config(
    ...
    let project_dir = loopcut_core::paths::default_screenpipe_data_dir()
        .join("pi-chat")
        .to_string_lossy()
        .to_string();
...
pub fn ensure_pi_installed_background() {
    ...
    #[cfg(windows)]
    {
        let _ = std::thread::Builder::new()
            ...
            .spawn(|| {
                loopcut_core::agents::pi::ensure_bash_available();
            });
    }
