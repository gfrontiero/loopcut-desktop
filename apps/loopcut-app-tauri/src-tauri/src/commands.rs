...
pub fn get_enterprise_license_key() -> Option<String> {
    ...
    let user_path = loopcut_core::paths::default_screenpipe_data_dir().join("enterprise.json");
    if user_path.exists() {
        info!(
...
pub fn save_enterprise_license_key(license_key: String) -> Result<(), String> {
    let dir = loopcut_core::paths::default_screenpipe_data_dir();
...
pub async fn get_disk_usage(
    ...
    let screenpipe_dir_path = match data_dir {
        ...
        _ => loopcut_core::paths::default_screenpipe_data_dir(),
    };
...
pub async fn perform_ocr_on_image(
    ...
    let languages: Vec<loopcut_core::Language> = store
        .recording
        .languages
        .iter()
        .filter(|s| s != &"default")
    ...
    let (text, _text_json, _confidence) = match ocr_engine {
        #[cfg(target_os = "macos")]
        OcrEngine::AppleNative => loopcut_screen::perform_ocr_apple(&img, &languages),
        OcrEngine::Tesseract => loopcut_screen::perform_ocr_tesseract(&img, languages),
        #[cfg(target_os = "windows")]
        OcrEngine::WindowsNative => loopcut_screen::perform_ocr_windows(&img)
            .await
...
pub async fn list_cache_files() -> Result<Vec<CacheFile>, String> {
    let data_dir = loopcut_core::paths::default_screenpipe_data_dir();
