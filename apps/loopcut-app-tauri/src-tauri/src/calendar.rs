            ...
            if format!("{}", status) == "Full Access" {
                ...
                .await
                {
                    Ok(Ok(events)) => {
                        ...
                        if let Err(e) = loopcut_events::send_event("calendar_events", items) {
...
#[cfg(target_os = "macos")]
fn calendar_event_to_item(event: loopcut_connect::calendar::CalendarEvent) -> CalendarEventItem {
...
fn calendar_event_to_item_win(
    event: loopcut_connect::calendar_windows::CalendarEvent,
) -> CalendarEventItem {
    let start_display = event.start_local.format("%-I:%M %p").to_string();
    let end_display = event.end_local.format("%-I:%M %p").to_string();
