    ...
    pub async fn start_scheduler(&mut self) -> Result<()> {
        ...
        let handle = tokio::spawn(async move {
            ...
            let mut meeting_start_rx =
                loopcut_events::subscribe_to_event::<serde_json::Value>("meeting_started");
            let mut meeting_end_rx =
                loopcut_events::subscribe_to_event::<serde_json::Value>("meeting_ended");
            let mut workflow_rx =
                loopcut_events::subscribe_to_event::<serde_json::Value>("workflow_event");
            // pipe_completed:* uses subscribe_to_all with prefix filtering below
            let mut pipe_completed_rx = loopcut_events::subscribe_to_all_events();
            ...
            loop {
                ...
                for (name, config, body) in &pipe_snapshot {
                    ...
                    tokio::spawn(async move {
                        ...
                        let _ = loopcut_events::send_event(
                            &event_name,
                            loopcut_events::PipeCompletedEvent {
