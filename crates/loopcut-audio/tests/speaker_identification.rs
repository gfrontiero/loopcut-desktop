mod tests {
    ...
    use loopcut_audio::speaker::segment::get_segments;
    ...
    async fn test_audio_speaker_identification() {
        ...
        let (mut obama_data, obama_sample_rate) =
            loopcut_audio::pcm_decode(&obama_path).expect("Failed to decode audio file");

        // reesample if not 16000
        if obama_sample_rate != 16000 {
            obama_data = loopcut_audio::resample(&obama_data, obama_sample_rate, 16000).unwrap();
        }
