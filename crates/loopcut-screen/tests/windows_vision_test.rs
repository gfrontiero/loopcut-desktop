...
mod tests {
    use chrono::Utc;
    use loopcut_screen::capture_screenshot_by_window::{CapturedWindow, WindowFilters};
    use loopcut_screen::core::RawCaptureResult;
    use loopcut_screen::monitor::get_default_monitor;
    use loopcut_screen::ocr_cache::WindowOcrCache;
    use loopcut_screen::{process_ocr_task, OcrEngine};
    use std::sync::Arc;
    use std::{path::PathBuf, time::Duration as StdDuration, time::Instant};
    use tokio::sync::{mpsc, Mutex};

    use loopcut_screen::{continuous_capture, PipelineMetrics};
