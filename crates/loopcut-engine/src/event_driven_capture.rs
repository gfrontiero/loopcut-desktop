...
use anyhow::Result;
use chrono::Utc;
use loopcut_a11y::tree::TreeWalkerConfig;
use loopcut_a11y::ActivityFeed;
use loopcut_db::DatabaseManager;
use loopcut_screen::capture_screenshot_by_window::{get_excluded_sck_window_ids, WindowFilters};
use loopcut_screen::frame_comparison::{FrameComparer, FrameComparisonConfig};
use loopcut_screen::monitor::SafeMonitor;
use loopcut_screen::snapshot_writer::SnapshotWriter;
use loopcut_screen::utils::capture_monitor_image;
...
pub struct CaptureParams<'a> {
    ...
    pub languages: &'a [loopcut_core::Language],
}
...
pub async fn event_driven_capture_loop(
    ...
    vision_metrics: Arc<loopcut_screen::PipelineMetrics>,
    hot_frame_cache: Option<Arc<HotFrameCache>>,
    use_pii_removal: bool,
    pause_on_drm_content: bool,
    languages: Vec<loopcut_core::Language>,
    ...
    let mut walk_budget = loopcut_a11y::budget::AppWalkBudget::new();
    ...
    loop {
        ...
        #[cfg(target_os = "macos")]
        if loopcut_screen::stream_invalidation::take() {
            info!(
            ...
            let invalidate_result = tokio::time::timeout(
                ...
                tokio::task::spawn_blocking(|| {
                    loopcut_screen::stream_invalidation::invalidate_streams();
                }),
            )
            .await;
...
fn resolve_capture_metadata(
    tree_snapshot: Option<&loopcut_a11y::tree::TreeSnapshot>,
    trigger: &CaptureTrigger,
...
async fn do_capture(
    ...
    walk_budget: &mut loopcut_a11y::budget::AppWalkBudget,
    ...
    use loopcut_a11y::tree::TreeWalkResult;
