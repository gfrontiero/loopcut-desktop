...
use loopcut_core::find_ffmpeg_path;
use loopcut_core::paths;
...
use loopcut_screen::monitor::list_monitors;
use serde_json::json;
use std::{
    env, fs,
    net::SocketAddr,
...
fn setup_logging(
    ...
    let file_appender = loopcut_engine::logging::SizedRollingWriter::builder()
...
async fn main() -> anyhow::Result<()> {
    ...
    let record_args = match cli.command {
        ...
        Command::Connection { ref subcommand } => {
            loopcut_engine::cli::connection::handle_connection_command(subcommand).await?;
            return Ok(());
        }
        Command::Vault { ref subcommand } => {
            loopcut_engine::cli::vault::handle_vault_command(subcommand).await?;
            return Ok(());
        }
        Command::Login => {
            loopcut_engine::cli::login::handle_login_command().await?;
            return Ok(());
        }
        Command::Whoami => {
            loopcut_engine::cli::login::handle_whoami_command().await?;
            return Ok(());
        }
        Command::Doctor => {
            ...
            let perms_ok = loopcut_core::permissions::preflight_check(true, true);
            eprintln!();
    ...
    let audio_manager = match audio_manager_builder.build(db.clone()).await {
        Ok(mut manager) => {
            ...
            manager.set_on_transcription_insert(std::sync::Arc::new(move |info| {
                ...
                rt.spawn(async move {
                    use loopcut_engine::hot_frame_cache::HotAudio;
                    cache
                        .push_audio(HotAudio {
    ...
    pipe_manager.set_token_registry(std::sync::Arc::new(
        loopcut_engine::pipe_permissions_middleware::DashMapTokenRegistry::new(
