...
use loopcut_connect::connections::ConnectionManager;
use loopcut_connect::oauth::{self as oauth_store, PENDING_OAUTH};
use loopcut_connect::whatsapp::WhatsAppGateway;
...
async fn list_connections(State(state): State<ConnectionsState>) -> Json<Value> {
    ...
    let wa_connected = matches!(
        wa_status,
        loopcut_connect::whatsapp::WhatsAppStatus::Connected { .. }
    );
...
fn get_native_calendar_events(hours_back: i64, hours_ahead: i64) -> Result<Vec<Value>, String> {
    use loopcut_connect::calendar::ScreenpipeCalendar;
...
fn get_native_calendar_events(hours_back: i64, hours_ahead: i64) -> Result<Vec<Value>, String> {
    use loopcut_connect::calendar_windows::ScreenpipeCalendar;
    let cal = ScreenpipeCalendar::new()?;
...
#[cfg(target_os = "windows")]
fn is_native_calendar_available() -> bool {
    loopcut_connect::calendar_windows::ScreenpipeCalendar::is_available()
}
