...
fn default_audio_engine() -> CliAudioTranscriptionEngine {
    let tier = loopcut_config::detect_tier();
    if matches!(tier, loopcut_config::DeviceTier::Low) {
...
impl RecordArgs {
    ...
    pub fn to_recording_settings(&self) -> loopcut_config::RecordingSettings {
        ...
        loopcut_config::RecordingSettings {
    ...
    pub fn into_recording_config(
        ...
        if settings.device_tier.is_none() {
            ...
            let existing = loopcut_config::load_toml(&config_path).ok();
            ...
            if has_tier {
                ...
                if is_fresh {
                    loopcut_config::apply_tier_defaults(&mut settings, tier);
        ...
        let tier = settings
            ...
            .unwrap_or_else(loopcut_config::detect_tier);
        if loopcut_config::is_engine_unsafe(&settings.audio_transcription_engine, tier) {
            let safe = loopcut_config::best_engine_for_platform(tier);
            eprintln!(
...
pub fn get_or_create_machine_id(override_id: Option<String>) -> String {
    ...
    loopcut_core::sync::get_or_create_machine_id()
}
