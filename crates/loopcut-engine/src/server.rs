...
use loopcut_audio::audio_manager::AudioManager;
use loopcut_core::sync::SyncServiceHandle;
...
pub struct AppState {
    ...
    pub vision_metrics: Arc<loopcut_screen::PipelineMetrics>,
    /// Audio pipeline metrics (shared across all devices)
    pub audio_metrics: Arc<loopcut_audio::metrics::AudioPipelineMetrics>,
    ...
    pub pipe_permissions:
        Arc<DashMap<String, Arc<loopcut_core::pipes::permissions::PipePermissions>>>,
    ...
    pub vault: loopcut_vault::VaultManager,
...
impl SCServer {
    ...
    pub fn new(
        ...
        SCServer {
            ...
            video_quality,
            pipe_manager: None,
            vision_metrics: Arc::new(loopcut_screen::PipelineMetrics::new()),
            audio_metrics,
    ...
    pub async fn start(self) -> Result<(), std::io::Error> {
        ...
        // Advertise via mDNS
        if let Err(e) = loopcut_connect::mdns::advertise(self.addr.port()) {
    ...
    pub async fn create_router(&self) -> Router {
        ...
        let app_state = Arc::new(AppState {
            ...
            vault: loopcut_vault::VaultManager::new(self.screenpipe_dir.clone()),
            manual_meeting: self
                .manual_meeting
                .clone()
        ...
        let cm: crate::connections_api::SharedConnectionManager = Arc::new(Mutex::new(
            loopcut_connect::connections::ConnectionManager::new(self.screenpipe_dir.clone()),
        ));
        let wa: crate::connections_api::SharedWhatsAppGateway = Arc::new(Mutex::new(
            loopcut_connect::whatsapp::WhatsAppGateway::new(self.screenpipe_dir.clone()),
        ));
        ...
        // Auto-reconnect WhatsApp if a previous session exists on disk
        {
            ...
            if wa_lock.has_session() {
                ...
                let bun_path =
                    loopcut_connect::whatsapp::which_bun().unwrap_or_else(|| "bun".to_string());
