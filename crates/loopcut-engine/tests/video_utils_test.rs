...
use loopcut_engine::video_utils::extract_frames_from_video;
use loopcut_screen::capture_screenshot_by_window::CapturedWindow;
#[cfg(target_os = "macos")]
use loopcut_screen::perform_ocr_apple;
...
async fn test_get_video_metadata() -> Result<()> {
    ...
    let metadata =
        loopcut_engine::video_utils::get_video_metadata(video_path.to_str().unwrap()).await?;
